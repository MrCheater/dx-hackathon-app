function verifyCommand(command) {
  if (!command.aggregateId)
    return Promise.reject('"aggregateId" argument is required')
  if (!command.aggregateName)
    return Promise.reject('"aggregateName" argument is required')
...
export default ({ store, bus, aggregates }) => {
  ...
  return command =>
    verifyCommand(command).then(() =>
      executors[command.aggregateName.toLowerCase()](command)
    )
}
